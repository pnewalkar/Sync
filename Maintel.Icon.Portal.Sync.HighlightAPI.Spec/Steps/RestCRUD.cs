...
namespace Maintel.Icon.Portal.Sync.HighlightAPI.Spec.Steps
{
    ...
    public class RestCRUD
    {
        ...
        [Given(@"I make a ""(.*)"" request to ""(.*)"" with payload ""(.*)""")]
        public void GivenIMakeARequestTo(string method, string uri, string payload)
        {
            WhenIMakeARequestTo(method, uri, payload);
        }

        [Given(@"I make a ""(.*)"" request to ""(.*)"" with payload ""(.*)"" enriching ""(.*)"" with return from last call")]
        public void GivenIMakeARequestTo(string method, string uri, string payload, string targetproperty)
        {
            WhenIMakeARequestTo(method, uri, payload, targetproperty);
        }
        ...
        [When(@"I make a ""(.*)"" request to ""(.*)"" with payload ""(.*)"" enriching ""(.*)"" with return from last call")]
        ...
        public void ThenReturnedStringShouldNotBeEmpty()
        {
            Assert.IsTrue(HTTPRest.ResponseString.Length > 0);
        }

        [Then(@"the returned string should contain a value of ""(.*)""")]
        public void ThenReturnedStringShouldContainAValueOf(string value)
        {
            Assert.IsTrue(value.Length > 0);
            Assert.IsTrue(HTTPRest.ResponseString.Length > 0);
            Assert.IsTrue(HTTPRest.ResponseString.Replace("\"", "").ToLower().Contains(value.ToLower()), $"'{HTTPRest.ResponseString}' does not contain value: '{value}'");
        }
